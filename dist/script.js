...
/* harmony import */ var _modules_getQuestions__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./modules/getQuestions */ "./src/js/modules/getQuestions.js");
/* harmony import */ var _modules_putQuestion__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./modules/putQuestion */ "./src/js/modules/putQuestion.js");


window.addEventListener('DOMContentLoaded', function () {
  'use strict';

  new _modules_getQuestions__WEBPACK_IMPORTED_MODULE_0__["default"]().init(); // new PutQuestions().init();
});

/***/ }),

/***/ "./src/js/modules/getQuestions.js":
/*!****************************************!*\
  !*** ./src/js/modules/getQuestions.js ***!
  \****************************************/
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return GetQuestions; });
...
/* harmony import */ var _putQuestion__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ./putQuestion */ "./src/js/modules/putQuestion.js");
...
var GetQuestions = /*#__PURE__*/function () {
  function GetQuestions() {
    _classCallCheck(this, GetQuestions);
  ...
  _createClass(GetQuestions, [{
    key: "checkLocalStorage",
    value: function checkLocalStorage() {
      if (localStorage.getItem('localStorageData')) {
        var array = JSON.parse(localStorage.getItem('localStorageData'));
        var q = array[Math.floor(Math.random() * array.length)];
        this.checkAvailableQuestion(q);
      } else {
        var _q = this.questions[Math.floor(Math.random() * this.questions.length)];
        this.checkAvailableQuestion(_q);
      }
    }
  }, {
    key: "checkAvailableQuestion",
    value: function checkAvailableQuestion(q) {
      this.right = q.right;
    ...
    value: function createTimer() {
      ...
      animation = requestAnimationFrame(function () {
        return timerAnimation.call(_this, _this);
      });
      ...
      function timerAnimation(obj) {
        ...
        if (obj.click == true) {
          ...
          requestAnimationFrame(function () {
            return timerAnimation.call(_this2, obj);
          });
        } else if (width >= 1920) {
          obj.gameOver();
        }
    ...
    key: "createDifficultCost",
    value: function createDifficultCost() {
      var difficultType = document.createElement('div');
    ...
    key: "createScoreAndHints",
    value: function createScoreAndHints() {
    ...
    value: function bindTriggers(right, difficult) {
      ...
      btns.forEach(function (btn) {
        btn.addEventListener('click', function (e) {
          ...
          _this3.click = true; // клик по кнопке ответа для остановки таймера
          ...
          setTimeout(function () {
            if (btn.id === right) {
              ...
              switch (difficult) {
                case 'easy':
                  _this3.playerScore += 5;
                  break;

                case 'medium':
                  _this3.playerScore += 10;
                  break;

                case 'hard':
                  _this3.playerScore += 20;
                  break;

                case 'expert':
                  _this3.playerScore += 40;
                  break;
              }

              document.querySelector('.score .count').textContent = "".concat(_this3.playerScore);
              setTimeout(function () {
                _this3.rightAnswersCounter++;
                _this3.questionCounter++;

                _this3.clearFields();

                _this3.checkLocalStorage();
              }, 2000);
            } else {
              btn.classList.remove('choised');
    ...
    key: "checkQuestionProgress",
    value: function checkQuestionProgress(difficult) {
      var _this4 = this;

      document.querySelectorAll('.num').forEach(function (item, i) {
        item.classList.remove('current');

        if (i == _this4.questionCounter - 2) {
          item.classList.add('complete');
        }

        if (item.getAttribute('id') == "".concat(_this4.questionCounter)) {
          item.classList.add('current');

          if (!_this4.switch) {
            document.querySelector('.progress-wrapper').style.left = "calc(50% - ".concat(_this4.left, "px)");
            _this4.left += 74;
          }
    ...
    key: "bindHints",
    value: function bindHints(right) {
      var _this5 = this;

      document.querySelectorAll('.hint').forEach(function (hint) {
        hint.addEventListener('click', function () {
          right = _this5.right;
          var arrAnswers = document.querySelectorAll('.answer');
          var rightNum = parseInt(right.replace(/\D+/, '')) - 1;
          ...
          switch (hint.classList[1]) {
            case 'half':
              var j, k;
              right = _this5.right; // Переопределяем правильный ответ в случае, если ответ был сменен подсказкой "switch"

              if (_this5.playerScore < 50) {
                break;
              }

              _this5.playerScore = _this5.playerScore - 50;
              ...
              function deleteFailAnswers() {
                ...
                if (j !== rightNum && k !== rightNum && j !== k) {
                  arrAnswers.forEach(function (item, i) {
                    if (i == j || i == k) {
                      item.textContent = '';
                    }
                  });
            ...
            case 'switch':
              if (_this5.playerScore < 30) {
                break;
              }

              _this5.playerScore = _this5.playerScore - 30;
              _this5.switch = true;

              _this5.clearFields();

              _this5.checkLocalStorage();
            ...
            case 'expert':
              ...
              if (_this5.playerScore < 100) {
                console.log(1);
                break;
              }

              _this5.playerScore = _this5.playerScore - 100;
              arrAnswers.forEach(function (item, i) {
                if (i === rightNum) {
          ...
          _this5.switch = false;
          document.querySelector('.score .count').textContent = "".concat(_this5.playerScore);
        });
      });
    }
  }, {
    ...
    value: function addQuestion() {
      document.querySelector('#addQuestion').addEventListener('click', function () {
        ...
        new _putQuestion__WEBPACK_IMPORTED_MODULE_7__["default"]().init();
      });
    }
  }, {
    key: "newGame",
    value: function newGame() {
      ...
      document.querySelector('#newGame').addEventListener('click', function () {
        _this6.questionCounter = 1;
        _this6.playerScore = 0;
        _this6.archiveQuestions = [];
        document.querySelector('.result').remove();
        document.querySelector('.score-and-hints').remove();
        document.querySelector('.type_wrapper').remove();
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return PutQuestions; });
...
/* harmony import */ var _getQuestions__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./getQuestions */ "./src/js/modules/getQuestions.js");
...
var PutQuestions = /*#__PURE__*/function () {
  function PutQuestions() {
    _classCallCheck(this, PutQuestions);
  ...
  _createClass(PutQuestions, [{
    ...
    value: function addQuestion() {
      ...
      document.querySelector('#addQuestion').addEventListener('click', function (e) {
        e.preventDefault();

        _this.checkLocalStorage();

        var questionName = document.querySelector('#question_name'),
        ...
        _this.questionNameValue = questionName.value;
        answers.forEach(function (answer) {
          _this.answersValue.push(answer.value);
        });
        rightAnswers.forEach(function (right) {
          if (right.checked) {
            _this.rightAnswerValue = right.value;
          }
        });
        difficult.forEach(function (dif) {
          if (dif.checked) {
            _this.difficultValue = dif.value;
          }
        });

        if (_this.questionNameValue == '' || _this.answersValue[0] == '' || _this.answersValue[1] == '' || _this.answersValue[2] == '' || _this.answersValue[3] == '' || _this.rightAnswerValue == null || _this.difficultValue == null) {
          ...
          _this.questions.push({
            question: _this.questionNameValue,
            a1: _this.answersValue[0],
            a2: _this.answersValue[1],
            a3: _this.answersValue[2],
            a4: _this.answersValue[3],
            right: _this.rightAnswerValue,
            difficult: _this.difficultValue,
            id: _this.questions.length
          });

          localStorage.setItem('localStorageData', JSON.stringify(_this.questions));
    ...
    value: function newGame() {
      document.querySelector('#newGame').addEventListener('click', function () {
        ...
        new _getQuestions__WEBPACK_IMPORTED_MODULE_2__["default"]().init();
      });
    }
  }, {
    key: "init",
